...
const SYSTEM_INSTRUCTION = {
    ...
    parts: [{
        text: "You are a legal advisor chat assistant specialized in corporate law in India. " +
              ...
              "For example, you can say: 'I am a legal advisor chat assistant specialized in corporate law in India. Please ask questions related to corporate law.' " +
              "If a user types 'okay' or 'ok', respond with: 'Alright! Let me know if you have any questions related to corporate law.'"
    }]
...
class ChatManager {
    ...
    static clearChats() {
        localStorage.removeItem("saved-chats"); // Remove from Local Storage
        chats = []; // Clear the in-memory chat list
        elements.chatContainer.innerHTML = ""; // Clear the UI chat display
    
        // Also clear the sidebar chat list (if applicable)
...
async function handleSubmit(e) {
    ...
    try {
        ...
        setTimeout(() => ChatManager.autoScroll(), 100);
...
function initializeEventListeners() {
    ...
    elements.textInput.addEventListener("keydown", async (e) => {
        if (e.key === "Enter" && !e.shiftKey) {
            ...
            const message = elements.textInput.value.trim();
            if (message) {
                await handleSubmit({ preventDefault: () => {} }); // Simulated event
            }
        }
    });
